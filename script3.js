...
function doPunch(index) {
  var punchX = -(mouseX - blockData.locationX[index]);
  var punchY = -(mouseY - blockData.locationY[index]);
...
function doWalls(index) {
  if (blockData.locationX[index]<10) {
    ...
    if (blockData.velocityX[index]<0) {
      blockData.velocityX[index] = (-blockData.velocityX[index])*bounciness;
    }
  }
  if (blockData.locationY[index]<10) {
    blockData.locationY[index] = 10;
    if (blockData.velocityY[index]<0) {
      blockData.velocityY[index] = (-blockData.velocityY[index])*bounciness;
    }
  }
...
function draw() {
  ...
  for (var i = 0; i < amount; i ++) {
    drawBlock(i);
    if (dist(mouseX, mouseY, blockData.locationX[i], blockData.locationY[i]) < 25&&mouseIsPressed) {
  doPunch(i);
}
  }
